...
from keras.layers import Input, CuDNNLSTM, Dense, TimeDistributed, Bidirectional, concatenate
...
def create_models(encoder_input_train, decoder_input_train, hyperparams):
    ...
    """
    latent_dim_encoder = hyperparams["latent_dim"]
    latent_dim_decoder = latent_dim_encoder + latent_dim_encoder
    ...
    encoder_lstm = Bidirectional(CuDNNLSTM(units = latent_dim_encoder, return_state = True, name = "encoder_lstm"))
    ...
    decoder_lstm = CuDNNLSTM(units = latent_dim_decoder, return_sequences = True, return_state = True, name = "decoder_lstm")
    decoder_dense_hidden = TimeDistributed(Dense(latent_dim_encoder, activation = "selu", kernel_initializer = 'lecun_normal', name = "decoder_dense_hidden_1"), name = "time_distributed_1")
    decoder_dense_hidden_2 = TimeDistributed(Dense(hidden_dense_dim, activation = "selu", kernel_initializer = 'lecun_normal', name = "decoder_dense_hidden_2"), name = "time_distributed_2")
    ...
    decoder_state_input_h = Input(shape=(latent_dim_decoder,), name = "inference_encoder_input_h")
    decoder_state_input_c = Input(shape=(latent_dim_decoder,), name = "inference_encoder_input_c")
