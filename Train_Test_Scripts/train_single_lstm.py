...
def create_models(X_train, y_train, hyperparams):
    ...
    output_feature_vect_lenght = y_train.shape[2]
    # LSTM inputs
    lstm_inputs = Input(shape = (None, input_feature_vect_length), name = "lstm_inputs")
    # CuDNNLSTM layer
    lstm = CuDNNLSTM(units = latent_dim, return_state = False, name = "lstm")
    # Time distributed dense for each out
    dense_hidden_1 = TimeDistributed(Dense(latent_dim, activation = "linear", name = "x_dense_hidden_1"), name = "time_distributed_x_1")
    dense_output = TimeDistributed(Dense(output_feature_vect_lenght, activation = "linear", name = "x_dense_output"), name = "time_distributed_x_1")
